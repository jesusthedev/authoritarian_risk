...
function drawTrend(){
  const byMonth={}; for(const e of STATE.entries){ const m=e.date.slice(0,7); (byMonth[m] ||= []).push(e); }
...
function renderCategories(byCat){
  ...
  for(const c of CATEGORIES){ const val=(byCat?.get(c.id)||0).toFixed(2); const el=document.createElement('div'); el.className='card'; el.innerHTML=`<div class="cat-h">${c.name}</div><div class="text-xs text-zinc-400">Weight ${c.weight} • Month score ${val}</div>`; grid.appendChild(el); }
}
...
function update(){ const {score,byCat}=computeComposite(STATE.month); drawGauge(score); renderCategories(byCat); renderTable(); drawTrend(); kpis(); }
